...
exec > >(tee -i logs/rename_branch_script.log)
exec 2>&1
...
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git checkout "$old_name"
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git checkout master
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git branch -m "$old_name" "$new_name"
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git push origin :"$old_name" "$new_name"
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git checkout "$new_name"
GIT_SSH_COMMAND="ssh -p 2222 -i ../gitolite" git push origin -u "$new_name"
